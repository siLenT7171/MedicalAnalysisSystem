...
• Регионов охвачено: {data['Регион'].nunique() if 'Регион' in data.columns else 0}"""
    ...
    def forecast_ml(self):
        ...
        try:
            ...
            # График 2: Расширенный анализ модели (3 варианта)
            if len(X_test) > 0 and len(y_test) > 0:
                # Вариант A: Scatter plot точности + метрики
                ax2.scatter(y_test, y_pred, alpha=0.7, color='#3498DB', s=100, 
                        edgecolors='white', linewidth=2, label='Предсказания')
                
                # Идеальная линия
                min_val = min(min(y_test), min(y_pred)) * 0.95
                max_val = max(max(y_test), max(y_pred)) * 1.05
                ax2.plot([min_val, max_val], [min_val, max_val], 'r-', 
                        linewidth=3, alpha=0.8, label='Идеальная линия')
                
                # Линия тренда
                z = np.polyfit(y_test, y_pred, 1)
                p = np.poly1d(z)
                ax2.plot(y_test, p(y_test), "--", color='orange', 
                        linewidth=2, alpha=0.8, label=f'Тренд (R²={r2:.3f})')
                
                # Доверительный интервал
                residuals = y_pred - y_test
                std_residuals = np.std(residuals)
                ax2.fill_between([min_val, max_val], 
                                [min_val - std_residuals, max_val - std_residuals],
                                [min_val + std_residuals, max_val + std_residuals],
                                alpha=0.2, color='gray', label='±1σ интервал')
                
                ax2.set_xlabel('Фактические значения', fontsize=12, fontweight='bold')
                ax2.set_ylabel('Предсказанные значения', fontsize=12, fontweight='bold')
                ax2.set_title('📊 Точность модели: Факт vs Прогноз', fontsize=14, fontweight='bold')
                ax2.legend(loc='upper left', frameon=True, fancybox=True, shadow=True)
                ax2.grid(True, alpha=0.3)
                
                # Добавляем статистику
                stats_text = f'Метрики качества:\nR² = {r2:.3f}\nMAE = {mae:.1f}\nRMSE = {np.sqrt(np.mean(residuals**2)):.1f}\nТочек: {len(y_test)}'
                ax2.text(0.02, 0.98, stats_text, transform=ax2.transAxes, fontsize=10,
                        verticalalignment='top', horizontalalignment='left',
                        bbox=dict(boxstyle='round,pad=0.5', facecolor='lightblue', alpha=0.8))

            elif len(monthly_data) >= 12:
                # Вариант B: Сезонный анализ остатков
                y_train_pred = model.predict(X_train)
                residuals = y_train - y_train_pred
                
                # Привязываем остатки к месяцам
                months = monthly_data['Месяц'].iloc[len(monthly_data)-len(residuals):].values
                
                # Группируем остатки по месяцам
                month_residuals = {}
                for month, residual in zip(months, residuals):
                    if month not in month_residuals:
                        month_residuals[month] = []
                    month_residuals[month].append(residual)
                
                # Создаем box plot по месяцам
                ...
                for month in range(1, 13):
                    if month in month_residuals and len(month_residuals[month]) > 0:
                        box_data.append(month_residuals[month])
                        box_labels.append(month_names[month-1])
                ...
                if box_data:
                    ...
                    ax2.set_ylabel('Остатки модели', fontsize=12, fontweight='bold')
                    ax2.set_title('📈 Сезонный анализ ошибок модели', fontsize=14, fontweight='bold')
                    ax2.grid(True, alpha=0.3)
                    
                    # Статистика остатков
                    all_residuals = np.concatenate(box_data)
                    stats_text = f'Анализ остатков:\nСреднее: {np.mean(all_residuals):.2f}\nСтд. откл.: {np.std(all_residuals):.2f}\nМин/Макс: {np.min(all_residuals):.1f}/{np.max(all_residuals):.1f}'
                    ax2.text(0.02, 0.98, stats_text, transform=ax2.transAxes, fontsize=10,
                            verticalalignment='top', horizontalalignment='left',
                            bbox=dict(boxstyle='round,pad=0.5', facecolor='lightyellow', alpha=0.8))
                else:
                    # Fallback к важности признаков если нет данных для box plot
                    self._plot_feature_importance_enhanced(ax2, model)
            else:
                # Вариант C: Улучшенная важность признаков
                self._plot_feature_importance_enhanced(ax2, model)
